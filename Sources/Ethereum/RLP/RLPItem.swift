...
public struct RLPItem {
    ...
    public init(data: Data) {
        self.init(valueType: .bytes(data))
    }
    
    public static func bytes(_ bytes: Data) -> RLPItem {
        return RLPItem(data: bytes)
    }
    
    public static func bytes(_ bytes: UInt8...) -> RLPItem {
        return RLPItem(data: Data(bytes))
    }
}
...
extension RLPItem: ExpressibleByStringLiteral {
    ...
    public init(stringLiteral value: String) {
        self.init(valueType: .bytes(value.data(using: .utf8)!))
    }
}
...
extension RLPItem: ExpressibleByIntegerLiteral {
    ...
    public init(integerLiteral value: UInt) {
        self.init(valueType: .bytes(value.data.trimmedLeadingZeros))
    }
}
...
extension RLPItem: ValueConvertible {
    ...
    public init(ethereumValue: Value) throws {
        let data = try EthData(ethereumValue: ethereumValue)
        self.init(data: data.data)
    }

    public func ethereumValue() -> Value {
        let encoder = RLPEncoder()
        let string = try? EthData(encoder.encode(self)).hex

...
open class RLPEncoder {
     ...
     */
    open func encode(_ value: RLPItem) throws -> Data {
    ...
    private func encodeArray(_ elements: [RLPItem]) throws -> Data {
        var bytes = Data()
        ...
        if combinedCount <= 55 {
            ...
            let length = UInt(bytes.count).data.trimmedLeadingZeros
